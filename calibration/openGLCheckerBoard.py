...
def plotEllipsoidMask(x,Cov):
    ...
    img = cv2.cvtColor(img,cv2.COLORjpg_RGB2GRAY);
    img[img>0.5] = 1;
    img[img<=0.5] = 0;
    return img;
...
def plotParticles(X):
    ...
    glDisable(GL_BLEND);
    glDisalbe(GL_PROGRAM_POINT_SIZE);
    glColor4f(1,1,1,1);
def plotFlip():
    global observerLoc;
    global rotation;
...
if __name__ == "__main__":
    ...
    #img = cv2.resize(img,(int(0.1*img.shape[0]),int(0.1*img.shape[1])))
    ...
    while True:
        #plotAxis();
        plotlandmarks([0,-5,0],[0,0,-1],[0,0,1000],img);
